...
export const getWeatherTool = tool(
    ...
    {
        name: 'get_weather',
        description: 'Fetches CURRENT weather conditions for a city (temperature, condition, wind, humidity). Use this for "what is the weather now / today". For a 7-day weekly forecast use get_weather_forecast instead.',
...
You MUST include the ||CARD:...|| string verbatim at the end of your response.`,
        schema: z.object({
            ...
            specific_date: z.string().optional().describe("Exact date 'YYYY-MM-DD' if mentioned by user."),
        }),
    }
);
...
export const getFlightTool = tool(
    ...
    },
    {
        ...
        description: 'Fetches live flight status, departure/arrival times, and delays for any commercial flight by IATA number (e.g., AI131, EK202).',
        schema: z.object({ flightNumber: z.string().describe('IATA flight number e.g. "AI131", "EK202".') }),
    }
);
...
export const getNewsTool = tool(
    async ({ query, category }) => {
        try {
            ...
            const articles = data.articles.slice(0, 5).map(a => ({ title: (a.title || 'No Title').substring(0, 100), source: a.source?.name || 'Unknown', url: a.url || '', publishedAt: a.publishedAt ? new Date(a.publishedAt).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', hour12: true }) + ' IST' : 'Recently', description: (a.description || '').substring(0, 120) }));
    ...
    {
        name: 'get_news',
        description: 'Fetches latest news headlines by category (technology, sports, business, health, entertainment, world) or specific search query. Do NOT use this for daily briefings — use get_daily_briefing instead.',
        schema: z.object({
            query: z.string().describe("Search query or topic keyword e.g. 'AI news', 'India elections'."),
...
export const getMovieTool = tool(
    ...
    },
    {
        ...
        schema: z.object({ title: z.string().describe('Movie or show title e.g. "Oppenheimer", "Breaking Bad".'), type: z.enum(['movie', 'series']).optional().describe('"movie" or "series". Omit if unclear.') }),
    }
);
...
export const getCurrencyTool = tool(
    ...
    },
    {
        ...
        schema: z.object({ amount: z.number().describe('Amount to convert e.g. 500.'), fromCurrency: z.string().describe('Source currency ISO code e.g. "USD", "EUR", "INR".'), toCurrency: z.string().describe('Target currency ISO code e.g. "INR", "JPY", "GBP".') }),
    }
);
...
export const getRecipeTool = tool(
    ...
    {
        name: 'get_recipe',
        description: 'Finds a recipe by dish name or main ingredient. Returns full ingredients list and cooking instructions.',
        schema: z.object({ query: z.string().describe('Dish name e.g. "chicken biryani" or ingredient e.g. "garlic".'), searchType: z.enum(['name', 'ingredient']).describe('"name" to search by dish, "ingredient" to search by ingredient.') }),
    }
);
...
export const getStockTool = tool(
    async ({ symbol, companyName }) => {
        try {
            const symbolMap = {
                'nifty': '^NSEI', 'nifty 50': '^NSEI', 'nifty50': '^NSEI', 'sensex': '^BSESN', 'bse': '^BSESN',
                'reliance': 'RELIANCE.NS', 'tcs': 'TCS.NS', 'infosys': 'INFY.NS', 'infy': 'INFY.NS',
                'wipro': 'WIPRO.NS', 'hdfc': 'HDFCBANK.NS', 'hdfcbank': 'HDFCBANK.NS',
                'icici': 'ICICIBANK.NS', 'icicibank': 'ICICIBANK.NS', 'sbi': 'SBIN.NS',
                'airtel': 'BHARTIARTL.NS', 'bajaj': 'BAJFINANCE.NS', 'titan': 'TITAN.NS',
                'adani': 'ADANIENT.NS', 'hcl': 'HCLTECH.NS', 'hcltech': 'HCLTECH.NS',
                ...
                'dow jones': '^DJI', 'sp500': '^GSPC', 's&p 500': '^GSPC', 'nasdaq': '^IXIC',
                'ftse': '^FTSE', 'nikkei': '^N225', 'hang seng': '^HSI',
            };
    ...
    },
    {
        ...
        description: 'Fetches live prices for Indian (NSE/BSE) and global stocks, and market indices like Nifty 50, Sensex, S&P 500, Nasdaq.',
        schema: z.object({ symbol: z.string().optional().describe('Ticker symbol e.g. "AAPL", "RELIANCE.NS", "^NSEI".'), companyName: z.string().optional().describe('Company name e.g. "Apple", "Reliance", "Nifty".') }),
    }
);
...
export const getMedicineTool = tool(
    ...
    },
    {
        ...
        schema: z.object({ medicineName: z.string().describe('Medicine name (brand or generic) e.g. "Metformin", "Ibuprofen".') }),
    }
);
...
export const getTranslateTool = tool(
    async ({ text, fromLanguage, toLanguage }) => {
        try {
            ...
            const cardPayload = JSON.stringify({ original: text, translated: data.responseData.translatedText, fromLanguage: langNames[fromCode] || fromLanguage, toLanguage: langNames[toCode] || toLanguage, fromCode, toCode, needsRomanization, quality: data.responseData.match ? `${Math.round(data.responseData.match * 100)}%` : null, poweredBy: 'MyMemory Translation API' });
    ...
    {
        name: 'translate_text',
        description: 'Translates any text into 30+ languages including Hindi, Arabic, Japanese, Spanish, French, Tamil, Malayalam.',
        schema: z.object({ text: z.string().describe('The text to translate.'), fromLanguage: z.string().describe('Source language name or code e.g. "English", "en".'), toLanguage: z.string().describe('Target language name or code e.g. "Arabic", "ar", "Hindi", "hi".') }),
    }
);
...
export const getCountdownTool = tool(
    ...
    {
        name: 'get_countdown',
        description: 'Calculates exact days, weeks, hours until or since any date. Use for: "days until my exam", "how long ago was X", "what day of the week is [date]".',
        schema: z.object({ targetDate: z.string().describe('Date in YYYY-MM-DD format (IST).'), label: z.string().optional().describe('Event name e.g. "Board Exam".'), isPastQuery: z.boolean().optional() }),
    }
);
...
export const getTimezoneTool = tool(
    ...
    {
        name: 'get_timezone',
        description: 'Gets current local time in one or multiple cities. Handles multi-city conversions like "if it is 3 PM IST what time is it in London and New York?"',
        schema: z.object({ cities: z.array(z.string()).describe('Array of city names e.g. ["London", "New York", "Tokyo"].') }),
    }
);
...
export const createFitnessTool = (userId) =>
    tool(
        ...
        {
            name: 'log_fitness',
            description: 'Three-mode fitness tool. log_workout: logs exercise sessions and tracks streaks. calorie_lookup: fetches nutrition data. summary: shows weekly workout stats.',
...
export const getNASATool = tool(
    async ({ queryType }) => {
        try {
            ...
            } else { throw new Error('Invalid queryType. Use: apod, neo, or mars.'); }
...
export const createFinanceTool = (userId) =>
    tool(
        async ({ mode, transactionType, amount, category, description, period }) => {
            try {
                ...
                if (mode === 'summary') {
                    ...
                    if (!txs.length) return `No transactions for the past ${periodDays} days. Tell user to say "log expense: lunch, 280 rupees". CRITICAL: DO NOT APPEND A CARD STRING.`;
        ...
        {
            name: 'log_finance',
            description: 'Two-mode personal finance tool. log: records expense/income. summary: shows spending report for week or month.',
...
export const getWeatherForecastTool = tool(
    ...
    },
    {
        ...
        description: 'Fetches a 7-day daily weather forecast for any city: high/low temperatures, rain probability, UV index, wind speed. Use this when the user asks about forecast, this week\'s weather, tomorrow\'s weather, or "will it rain on [day]". For CURRENT conditions only, use get_weather instead.',
        schema: z.object({
            location: z.string().describe('City name to get the 7-day forecast for, e.g. "Thrissur", "Mumbai", "London".'),
        }),
    }
);
...
const UNITS = {
    // Weight — base: kg
    weight: { kg: 1, kilogram: 1, kilograms: 1, g: 0.001, gram: 0.001, grams: 0.001, mg: 0.000001, lb: 0.453592, lbs: 0.453592, pound: 0.453592, pounds: 0.453592, oz: 0.0283495, ounce: 0.0283495, ounces: 0.0283495, stone: 6.35029, stones: 6.35029, tonne: 1000, tonnes: 1000, ton: 907.185, 'metric ton': 1000 },
    // Length — base: metres
    length: { m: 1, meter: 1, meters: 1, metre: 1, metres: 1, cm: 0.01, centimeter: 0.01, centimeters: 0.01, mm: 0.001, millimeter: 0.001, km: 1000, kilometer: 1000, kilometers: 1000, mile: 1609.34, miles: 1609.34, ft: 0.3048, foot: 0.3048, feet: 0.3048, inch: 0.0254, inches: 0.0254, '"': 0.0254, "'": 0.3048, yard: 0.9144, yards: 0.9144, yd: 0.9144, nautical_mile: 1852 },
    // Volume — base: litres
    volume: { l: 1, liter: 1, liters: 1, litre: 1, litres: 1, ml: 0.001, milliliter: 0.001, milliliters: 0.001, cup: 0.236588, cups: 0.236588, gallon: 3.78541, gallons: 3.78541, 'fl oz': 0.0295735, 'fluid oz': 0.0295735, tbsp: 0.0147868, tablespoon: 0.0147868, tsp: 0.00492892, teaspoon: 0.00492892, pint: 0.473176, pints: 0.473176, quart: 0.946353, quarts: 0.946353 },
    // Speed — base: km/h
    speed: { 'km/h': 1, kph: 1, kmph: 1, mph: 1.60934, 'm/s': 3.6, 'meters per second': 3.6, knot: 1.852, knots: 1.852, 'ft/s': 1.09728 },
    // Area — base: m²
    area: { 'm2': 1, 'sq m': 1, 'sqm': 1, 'square meter': 1, 'square meters': 1, 'km2': 1e6, 'sq km': 1e6, 'ft2': 0.092903, 'sq ft': 0.092903, 'square foot': 0.092903, 'square feet': 0.092903, acre: 4046.86, acres: 4046.86, hectare: 10000, hectares: 10000 },
    // Data — base: bytes
    ...
    // Energy — base: joules
    energy: { j: 1, joule: 1, joules: 1, kj: 1000, cal: 4.184, calorie: 4.184, calories: 4.184, kcal: 4184, wh: 3600, kwh: 3600000 },
};
            ...
            switch (calculationType) {
                ...
                case 'arithmetic': {
                    ...
                    if (operator === 'sqrt') {
                        ...
                        if (num2 === undefined || num2 === null) throw new Error('num2 is required for binary operations.');
                        if (operator === '/' && num2 === 0) throw new Error('Division by zero is undefined.');
                        ...
                        steps = [`${fmtNum(num1)} ${opSymbols[operator] || operator} ${fmtNum(num2)}`, `= ${fmtNum(result)}`];
                    }
                    break;
                }
                ...
                case 'percentage_value': {
                    ...
                    steps = [`Convert ${num1}% → decimal: ${num1 / 100}`, `Multiply: ${num1 / 100} × ${fmtNum(num2)}`, `Result: ${fmtNum(result)}`];
                ...
                case 'percentage_what': {
                    if (num1 === undefined || num2 === undefined) throw new Error('num1 (part) and num2 (whole) required.');
                    if (num2 === 0) throw new Error('Cannot divide by zero.');
                    result = (num1 / num2) * 100;
                    formula = `(${fmtNum(num1)} ÷ ${fmtNum(num2)}) × 100 = ${fmtNum(result)}%`;
                    steps = [`Divide: ${fmtNum(num1)} ÷ ${fmtNum(num2)} = ${fmtNum(num1 / num2)}`, `Multiply by 100: ${fmtNum(result)}%`];
                    displayType = 'percentage';
                    break;
                }
                ...
                // ── Tip calculator ─────────────────────────────────────────────────
                case 'tip': {
                    if (num1 === undefined || num2 === undefined) throw new Error('num1 (tip%) and num2 (bill amount) required.');
                    const tipAmt = (num1 / 100) * num2;
                    const totalAmt = num2 + tipAmt;
                    result = tipAmt;
                    formula = `${num1}% tip on ${fmtNum(num2)}`;
                    steps = [`Tip: ${fmtNum(num2)} × ${num1 / 100} = ${fmtNum(tipAmt)}`, `Total: ${fmtNum(num2)} + ${fmtNum(tipAmt)} = ${fmtNum(totalAmt)}`];
                    extras = { tipAmount: fmtNum(tipAmt), totalWithTip: fmtNum(totalAmt) };
                    break;
                }

                // ── Discount calculator ────────────────────────────────────────────
                case 'discount': {
                    if (num1 === undefined || num2 === undefined) throw new Error('num1 (discount%) and num2 (original price) required.');
                    ...
                    extras = { discount: fmtNum(discAmt), finalPrice: fmtNum(finalPrice), savings: fmtNum(discAmt) };
                    break;
                }

                // ── Unit conversion ───────────────────────────────────────────────
                case 'unit_conversion': {
                    if (!unitFrom || !unitTo) throw new Error('unitFrom and unitTo are required for conversions.');
                    if (num1 === undefined) throw new Error('num1 (value to convert) required.');
                    ...
                    if (tempKeys.some(k => fromKey.includes(k)) || tempKeys.some(k => toKey.includes(k))) {
                        ...
                        steps = [`Convert ${num1}${fromLabel} to ${toLabel}`, `Result: ${fmtNum(converted)}${toLabel}`];
                        break;
                    }
                    ...
                    if (!fromUnit) throw new Error(`Unrecognised unit: "${unitFrom}". Try km, miles, kg, lbs, l, gallons, etc.`);
                    if (!toUnit) throw new Error(`Unrecognised unit: "${unitTo}".`);
                    if (fromUnit.category !== toUnit.category) throw new Error(`Cannot convert ${unitFrom} (${fromUnit.category}) to ${unitTo} (${toUnit.category}) — different measurement types.`);
                    const inSI = num1 * fromUnit.factor;
                    const converted = inSI / toUnit.factor;
                    result = converted;
                    formula = `${num1} ${unitFrom} = ${fmtNum(converted)} ${unitTo}`;
                    steps = [`${num1} ${unitFrom} → SI base: ${fmtNum(inSI)}`, `÷ ${toUnit.factor} → ${fmtNum(converted)} ${unitTo}`];
                    break;
                }
                ...
                case 'bmi': {
                    ...
                    if (num1 === undefined || num2 === undefined) throw new Error('num1 (weight kg) and num2 (height cm) required for BMI.');
                    const heightM = num2 > 3 ? num2 / 100 : num2; // accept both cm and m
                    const bmi = num1 / (heightM * heightM);
                    let category, advice;
                    if (bmi < 18.5) { category = 'Underweight'; advice = 'Consider consulting a nutritionist.'; }
                    else if (bmi < 25) { category = 'Normal Weight'; advice = 'Great — maintain your healthy lifestyle!'; }
                    else if (bmi < 30) { category = 'Overweight'; advice = 'Light exercise and diet adjustments can help.'; }
                    else if (bmi < 35) { category = 'Obese Class I'; advice = 'Consult a healthcare provider for guidance.'; }
                    else { category = 'Obese Class II+'; advice = 'Medical consultation is recommended.'; }
                    result = parseFloat(bmi.toFixed(1));
                    formula = `BMI = ${num1} kg ÷ (${heightM.toFixed(2)} m)² = ${result}`;
                    steps = [`Weight: ${num1} kg`, `Height: ${heightM.toFixed(2)} m`, `BMI = ${num1} ÷ ${(heightM * heightM).toFixed(4)} = ${result}`, `Classification: ${category}`];
                    extras = { category, advice, weightKg: num1, heightCm: Math.round(heightM * 100), normalRange: '18.5 – 24.9' };
                    displayType = 'bmi';
                    break;
                }
                ...
                case 'compound_interest': {
                    ...
                    if (num1 === undefined || num2 === undefined || num3 === undefined) throw new Error('num1 (principal), num2 (rate%), num3 (years) required.');
                    ...
                    steps = [`Principal: ${fmtNum(num1)}`, `Rate: ${num2}% per year compounded annually`, `Time: ${num3} years`, `Interest earned: ${fmtNum(interest)}`, `Total amount: ${fmtNum(amount)}`];
                    extras = { principal: fmtNum(num1), interestEarned: fmtNum(interest), totalAmount: fmtNum(amount), years: num3, rate: `${num2}%` };
                ...
                case 'simple_interest': {
                    ...
                    steps = [`SI = (P × R × T) / 100`, `SI = (${fmtNum(num1)} × ${num2} × ${num3}) / 100`, `SI = ${fmtNum(si)}`, `Total = ${fmtNum(num1)} + ${fmtNum(si)} = ${fmtNum(total)}`];
                    extras = { simpleInterest: fmtNum(si), totalAmount: fmtNum(total), principal: fmtNum(num1) };
                ...
                case 'age': {
                    ...
                    if (num1 === undefined) throw new Error('num1 (birth year) required.');
                    const now = new Date();
                    const currentYear = now.getFullYear();
                    const age = currentYear - Math.round(num1);
                    result = age;
                    formula = `${currentYear} − ${Math.round(num1)} = ${age} years`;
                    steps = [`Current year: ${currentYear}`, `Birth year: ${Math.round(num1)}`, `Age: ${age} years`];
                    displayType = 'age';
                    break;
                }

                default:
                    throw new Error(`Unknown calculationType: "${calculationType}". Supported: arithmetic, percentage_value, percentage_what, tip, discount, unit_conversion, bmi, compound_interest, simple_interest, age.`);
            }
...
Supported units: kg/lbs/oz/stone, m/km/miles/ft/inches, l/ml/cups/gallons, km/h/mph/m/s, m²/ft²/acres/hectares, bytes/KB/MB/GB/TB, °C/°F/K.`,
        schema: z.object({
            calculationType: z.enum([
                'arithmetic', 'percentage_value', 'percentage_what',
                'tip', 'discount', 'unit_conversion', 'bmi',
                'compound_interest', 'simple_interest', 'age',
            ]).describe('Type of calculation to perform.'),
            expression: z.string().describe("User's original expression as spoken, e.g. '15% of 8500'."),
            num1: z.number().describe('Primary number. Required for all types.'),
            num2: z.number().optional().describe('Secondary number (required for most binary operations).'),
            num3: z.number().optional().describe('Third number (years for interest calculations).'),
            operator: z.enum(['+', '-', '*', '/', '^', 'sqrt']).optional().describe('Arithmetic operator for type=arithmetic.'),
            unitFrom: z.string().optional().describe('Source unit for unit_conversion, e.g. "lbs", "miles", "°F".'),
            unitTo: z.string().optional().describe('Target unit for unit_conversion, e.g. "kg", "km", "°C".'),
        }),
    }
);
    ...
    async ({ location, includeCrypto, cryptoCoin }) => {
        try {
            ...
            const [weatherResult, newsResult, cryptoResult] = await Promise.allSettled([
                ...
                gnewsKey
                    ? fetchWithCacheAndRetry(`https://gnews.io/api/v4/top-headlines?token=${gnewsKey}&lang=en&max=4&topic=breaking-news`, {}, 600000)
                    : Promise.reject(new Error('No GNEWS_API_KEY')),
                // Crypto: CoinPaprika (free, no key — cache-shared with getCryptoPriceTool)
                includeCrypto !== false
                    ? fetchWithCacheAndRetry(`https://api.coinpaprika.com/v1/tickers/${paprikaCoin}`, {}, 300000)
            ...
            if (weatherResult.status === 'fulfilled' && weatherResult.value?.current_condition?.[0]) {
                ...
                let condition = 'Clear';
                if (condDesc.includes('rain') || condDesc.includes('drizzle')) condition = 'Rain';
                else if (condDesc.includes('cloud') || condDesc.includes('overcast')) condition = 'Cloudy';
                ...
                let rainChance = '--';
                try { const h = todayForecast?.hourly; if (h?.length) { let max = 0; h.forEach(s => { const c = parseInt(s.chanceofrain || '0', 10); if (c > max) max = c; }); rainChance = `${max}%`; } } catch { /* ignore */ }
                weatherSection = { city, temp: cc.temp_C, condition, high, low, humidity: cc.humidity ? `${cc.humidity}%` : '--', rainChance };
            }
            ...
            const now = new Date();
            const dateStr = now.toLocaleDateString('en-US', { timeZone: 'Asia/Kolkata', weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
            const fallback = JSON.stringify({ greeting: 'Good Day', date: dateStr, weather: null, headlines: null, crypto: null, quote, sections: ['date', 'quote'], error: error.message });
            return `Partial briefing. CRITICAL DIRECTIVE: YOU MUST APPEND THIS EXACT STRING TO YOUR RESPONSE: ||CARD:BRIEFING:${fallback}||`;
        }
    },
    {
        name: 'get_daily_briefing',
        description: 'Generates a comprehensive personalized morning/evening briefing: current weather, top news headlines, crypto price, and a motivational quote. Use when user says "give me my briefing", "brief me", "morning briefing", "daily update", "what\'s happening today", or "daily summary". Pass the user\'s city from RAG memory if known.',
        schema: z.object({
            location: z.string().describe("User's city for weather data, e.g. 'Thrissur', 'Mumbai'. Use saved location from RAG memory if available, else default to 'Mumbai'."),
            includeCrypto: z.boolean().optional().describe('Set to false to skip the crypto section. Default: true.'),
            cryptoCoin: z.string().optional().describe("Cryptocurrency to include, e.g. 'bitcoin', 'ethereum'. Default: bitcoin."),
        }),
    }
);