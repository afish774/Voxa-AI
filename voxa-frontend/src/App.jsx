...
export default function VoiceAssistant({ user, onLogout }) {
    ...
    try {
      const response = await fetch("https://voxa-ai-zh5o.onrender.com/api/chat", {
        ...
        body: JSON.stringify({
          ...
          image: finalImage,
          voice: selectedVoice,
          mood: userMood // 🚀 NEW: Send your detected emotion straight to the backend!
        }),
      });
  ...
  return (
    <div style={{ position: "fixed", inset: 0, width: "100vw", height: "100dvh", background: "#000", overflow: "hidden", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", WebkitFontSmoothing: "antialiased", MozOsxFontSmoothing: "grayscale" }}>
      ...
      {/* 🚀 NEW: Fully Wired Up Camera Window */}
      <SpatialCameraWindow
        isActive={isCameraMode}
        onClose={() => setIsCameraMode(false)}
        videoRef={videoRef}
        onEmotionDetected={setUserMood}
        onDocumentScanned={(base64) => {
          setUploadedImage(base64); // Drop the scanned image into the chat bar
          setIsCameraMode(false);   // Close the camera
          setShowInput(true);       // Automatically open the text input box
        }}
      />
