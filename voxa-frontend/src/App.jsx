...
function QuerySlider({ theme, onSelect }) {
  ...
  return (
    <div style={{ position: "relative", height: "clamp(28px,3.6vw,44px)", overflow: "hidden", width: "100%", marginTop: "0.8em", cursor: "pointer" }} onClick={() => onSelect(SAMPLE_QUERIES[idx])}>
      <AnimatePresence mode="wait">
        <motion.p key={idx} initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -26 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} style={{ position: "absolute", width: "100%", textAlign: "inherit", fontSize: "clamp(12px,1.7vw,18px)", color: theme.textMuted, fontWeight: 400, letterSpacing: "-0.01em", lineHeight: 1.4, transition: "color 0.2s" }} onMouseEnter={(e) => e.target.style.color = theme.text} onMouseLeave={(e) => e.target.style.color = theme.textMuted}>
          "{SAMPLE_QUERIES[idx]}"
        </motion.p>
      </AnimatePresence>
    </div>
          ...
          <AnimatePresence mode="wait">
            {showGreeting && !isCameraMode && (
              <motion.div key="greeting" initial={{ opacity: 0, y: 20, filter: "blur(10px)" }} animate={{ opacity: greetingVisible ? 1 : 0, y: greetingVisible ? 0 : -30, filter: greetingVisible ? "blur(0px)" : "blur(12px)", scale: greetingVisible ? 1 : 0.9 }} exit={{ opacity: 0, y: -30, filter: "blur(12px)", scale: 0.9 }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }} style={{ width: "100%" }}>
                <p style={{ fontSize: "clamp(18px,2.8vw,26px)", color: theme.textMuted, fontWeight: 400, letterSpacing: "-0.01em", marginBottom: "0.2em" }}>{greetingText}, {userName?.split(' ')[0] || "Guest"}</p>
                ...
                <div style={{ minHeight: "clamp(36px, 6vw, 70px)", display: "flex", alignItems: "flex-end" }}>
                  <AnimatePresence mode="wait">
                    <motion.p
                      ...
                      transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
                      style={{ margin: 0, fontSize: "clamp(32px,5.8vw,64px)", color: theme.text, fontWeight: 400, letterSpacing: "-0.04em", lineHeight: 1.1, maxWidth: "min(900px, 85vw)", transformOrigin: effectiveSplit ? "left center" : "center" }}
                    >
                      {currentPrompt}
                    </motion.p>
                  </AnimatePresence>
                ...
                <AnimatePresence mode="wait">
                  {currentResponse && (
                    <motion.div
                      ...
                      transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
                    >
                      <p style={{ margin: 0, fontSize: "clamp(18px,2.8vw,26px)", color: theme.textMuted, fontWeight: 400, lineHeight: 1.4, letterSpacing: "-0.01em", maxWidth: "min(720px, 75vw)" }}>
