...
const RainEffect = ({ isThunder }) => {
  ...
  return (
    <>
      ...
      {isThunder && (
        <motion.div
          className="absolute inset-0 rounded-[36px] bg-white/80 pointer-events-none"
  ...
  <>
    ...
    <motion.div
      ...
      style={{
        ...
        height: '80%',
        borderRadius: '50%',
        background: 'radial-gradient(circle, rgba(251,191,36,0.25) 0%, rgba(245,158,11,0.08) 45%, transparent 70%)',
      }}
    ...
    <motion.div
      ...
      style={{
        ...
        height: '50%',
        borderRadius: '50%',
        background: 'radial-gradient(circle, rgba(251,191,36,0.35) 0%, rgba(253,224,71,0.10) 50%, transparent 75%)',
      }}
...
const CloudyEffect = () => {
  const clouds = useMemo(() => [
    { id: 0, w: 140, h: 48, top: '6%', from: -50, to: 320, dur: 24, op: 0.08 },
    { id: 1, w: 120, h: 40, top: '28%', from: 340, to: -60, dur: 30, op: 0.06 },
    { id: 2, w: 160, h: 54, top: '50%', from: -70, to: 330, dur: 36, op: 0.10 },
    { id: 3, w: 100, h: 34, top: '72%', from: 310, to: -40, dur: 28, op: 0.05 },
  ], []);
...
const WindyEffect = () => {
  ...
  return streaks.map(s => (
    <motion.div
      key={s.id}
      className="absolute bg-[#FFFFFF] rounded-full pointer-events-none"
...
// ═══════════════════════════════════════════════════════════════════
const SunIcon3D = () => (
  <motion.div className="relative flex items-center justify-center" style={{ width: 80, height: 80 }}>
    <motion.div
      className="absolute rounded-full"
      style={{
        width: 72, height: 72,
    ...
    <motion.span
      ...
      style={{ fontSize: 56, display: 'inline-block', filter: 'drop-shadow(0 6px 16px rgba(251,191,36,0.50)) drop-shadow(0 2px 4px rgba(0,0,0,0.20))' }}
      animate={{ rotate: 360 }}
...
const CloudIcon3D = () => (
  <motion.div className="relative flex items-center justify-center" style={{ width: 80, height: 80 }}>
    <motion.span
      className="relative z-10"
      style={{ fontSize: 56, display: 'inline-block', filter: 'drop-shadow(0 8px 20px rgba(148,163,184,0.50)) drop-shadow(0 2px 6px rgba(0,0,0,0.15))' }}
...
const RainIcon3D = () => (
  <motion.div className="relative flex items-center justify-center" style={{ width: 80, height: 90 }}>
    <motion.span
      className="relative z-10"
      style={{ fontSize: 48, display: 'inline-block', filter: 'drop-shadow(0 6px 18px rgba(100,116,139,0.50)) drop-shadow(0 2px 4px rgba(0,0,0,0.20))' }}
    ...
    {[0, 1, 2, 3, 4].map(i => (
      <motion.div
        ...
        className="absolute rounded-full"
        style={{
          width: 2, height: 8 + i * 2, bottom: 4, left: 18 + i * 10,
          background: 'linear-gradient(to bottom, rgba(96,165,250,0.7), rgba(59,130,246,0.2))',
          borderRadius: 2,
        }}
        animate={{ y: [0, 18, 0], opacity: [0, 0.8, 0] }}
        transition={{ duration: 0.8 + i * 0.15, delay: i * 0.18, repeat: Infinity, ease: 'easeIn' }}
      />
    ))}
  </motion.div>
);
...
const SnowIcon3D = () => (
  <motion.div className="relative flex items-center justify-center" style={{ width: 80, height: 80 }}>
    <motion.div
      className="absolute rounded-full"
      style={{ width: 64, height: 64, background: 'radial-gradient(circle, rgba(186,230,253,0.30) 0%, transparent 70%)', filter: 'blur(6px)' }}
      animate={{ scale: [1, 1.15, 1], opacity: [0.4, 0.7, 0.4] }}
      transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut' }}
    />
    <motion.span
      className="relative z-10"
      style={{ fontSize: 56, display: 'inline-block', filter: 'drop-shadow(0 6px 16px rgba(186,230,253,0.55)) drop-shadow(0 2px 4px rgba(0,0,0,0.15))' }}
      animate={{ rotate: 360, y: [-3, 3, -3] }}
      transition={{ rotate: { duration: 16, repeat: Infinity, ease: 'linear' }, y: { duration: 3, repeat: Infinity, ease: 'easeInOut' } }}
    >
      ❄️
    </motion.span>
  </motion.div>
...
const FogIcon3D = () => (
  <motion.div className="relative flex items-center justify-center" style={{ width: 80, height: 80 }}>
    <motion.span
      className="relative z-10"
      style={{ fontSize: 56, display: 'inline-block', filter: 'drop-shadow(0 6px 18px rgba(148,163,184,0.45)) drop-shadow(0 2px 4px rgba(0,0,0,0.12))' }}
      animate={{ y: [-3, 3, -3], x: [-3, 3, -3], opacity: [0.7, 1, 0.7] }}
...
const WindIcon3D = () => (
  <motion.div className="relative flex items-center justify-center" style={{ width: 80, height: 80 }}>
    <motion.span
      className="relative z-10"
      style={{ fontSize: 56, display: 'inline-block', filter: 'drop-shadow(0 6px 14px rgba(148,163,184,0.40)) drop-shadow(0 2px 4px rgba(0,0,0,0.15))' }}
...
// ═══════════════════════════════════════════════════════════════════
// ☀️  WeatherCard — Premium Crypto-Matched OS Widget
// ═══════════════════════════════════════════════════════════════════
export default function WeatherCard({
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-[380px] rounded-[36px] p-7 relative overflow-hidden select-none"
      style={{
        background: 'rgba(11, 11, 12, 0.85)', // Deep matte glass to match Crypto Card
    ...
    >
      {/* ─── Premium Unified Grid Background ─── */}
      <div
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        ...
        <div className="mb-6 flex flex-col">
          <h3 className="text-[#FFFFFF] font-semibold text-[18px] tracking-tight leading-tight">
            {_location}
          </h3>
          {_date && (
            <p className="text-[#A1A1AA] text-[12px] mt-[2px] tracking-wide font-medium uppercase">
              {_date}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex flex-col">
            <div className="leading-none flex items-start">
              <span className="text-[72px] font-light text-[#FFFFFF] tracking-tighter">
                {_temp ?? '--'}
              </span>
              <span className="text-[36px] font-light text-[#A1A1AA] mt-2">°</span>
            </div>
            <p className="text-[#F4F4F5] font-medium text-[15px] mt-1 tracking-wide capitalize">
              {_condition}
            </p>
          </div>
        </div>
        ...
        <div
          className="pt-5 grid grid-cols-3"
        ...
        >
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.04)' }}>
              <Wind className="w-4 h-4 text-[#A1A1AA]" strokeWidth={1.5} />
            </div>
            <span className="text-[#71717A] text-[10px] leading-none font-bold uppercase tracking-widest">Wind</span>
            <span className="text-[#F4F4F5] font-semibold text-[13px] leading-none">{_windSpeed}</span>
          </div>

          {/* Rain */}
          <div className="flex flex-col items-center gap-2" style={{ borderLeft: '1px solid rgba(255,255,255,0.06)', borderRight: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.04)' }}>
              <CloudRain className="w-4 h-4 text-[#A1A1AA]" strokeWidth={1.5} />
            </div>
            <span className="text-[#71717A] text-[10px] leading-none font-bold uppercase tracking-widest">Rain</span>
            <span className="text-[#F4F4F5] font-semibold text-[13px] leading-none">{_rainChance}</span>
          </div>

          {/* Humidity */}
          <div className="flex flex-col items-center gap-2">
            <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.04)' }}>
              <Droplets className="w-4 h-4 text-[#A1A1AA]" strokeWidth={1.5} />
            </div>
            <span className="text-[#71717A] text-[10px] leading-none font-bold uppercase tracking-widest">Humidity</span>
            <span className="text-[#F4F4F5] font-semibold text-[13px] leading-none">{_humidity}</span>
          </div>
        </div>
