...
const themeColors = {
    primary: "#7c3aed", secondary: "#db2777", gradient: "linear-gradient(135deg, #7c3aed, #db2777)",
    surface: "#ffffff", surfaceHover: "#fafafa", border: "rgba(0, 0, 0, 0.06)",
    textMain: "#09090b", textMuted: "#52525b", textFaint: "#a1a1aa"
};
        ...
        <span style={{ display: "inline-flex", flexWrap: "wrap", justifyContent: "center", ...style }}>
            {words.map((word, i) => (
                <span key={i} style={{ display: "inline-block", overflow: "hidden", marginRight: "0.2em", paddingBottom: "0.1em" }}>
                    <motion.span initial={{ y: "120%", rotateZ: 3, opacity: 0 }} animate={{ y: "0%", rotateZ: 0, opacity: 1 }} transition={{ duration: 0.8, ease: customEase, delay: delay + i * 0.06 }} style={{ display: "inline-block", transformOrigin: "bottom left", willChange: "transform, opacity" }}>{word}</motion.span>
                </span>
            ))}
        </span>
    );
...
function SpotlightCard({ children, className, style }) {
    ...
    return (
        <motion.div onMouseMove={handleMouseMove} whileHover={{ y: -4, boxShadow: "0 40px 80px -20px rgba(0,0,0,0.08), 0 10px 20px -5px rgba(0,0,0,0.04), inset 0 1px 0 rgba(255,255,255,1)" }} transition={springConfig} style={{ position: "relative", overflow: "hidden", background: "linear-gradient(180deg, #ffffff 0%, #fafafa 100%)", borderRadius: 36, border: `1px solid ${themeColors.border}`, boxShadow: "0 10px 30px -10px rgba(0,0,0,0.03), inset 0 1px 0 rgba(255,255,255,1)", willChange: "transform, box-shadow", ...style }} className={className}>
            <motion.div className="pointer-events-none absolute -inset-px opacity-0 transition duration-500" whileHover={{ opacity: 1 }} style={{ background: useMotionTemplate`radial-gradient(800px circle at ${mouseX}px ${mouseY}px, rgba(124, 58, 237, 0.04), transparent 60%)`, zIndex: 1, position: "absolute", inset: 0 }} />
            <div style={{ position: "relative", zIndex: 2, padding: "clamp(32px, 5vw, 48px)", height: "100%", display: "flex", flexDirection: "column" }}>{children}</div>
        </motion.div>
    );
}
                    ...
                    <div className="timeline-visual" style={{ display: "flex", justifyContent: "center" }}>
                        <motion.div style={{ width: "100%", aspectRatio: "3/4", background: "linear-gradient(180deg, #ffffff 0%, #f4f4f5 100%)", borderRadius: 36, padding: "clamp(10px, 2vw, 16px)", boxShadow: "0 40px 80px -20px rgba(0,0,0,0.1), inset 0 1px 1px rgba(255,255,255,1), 0 0 0 1px rgba(0,0,0,0.05)", position: "relative" }}>
                            ...
                            <div style={{ width: "100%", height: "100%", background: "#000", borderRadius: 24, overflow: "hidden", position: "relative", boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.1)" }}>
                                <AnimatePresence mode="wait">{renderTabletUI()}</AnimatePresence>
                            </div>
                        </motion.div>
                    </div>
                </div>
        ...
        <div style={{ minHeight: "100dvh", backgroundColor: "#fdfdfd", color: themeColors.textMain, fontFamily: "'SF Pro Display', 'Inter', -apple-system, BlinkMacSystemFont, sans-serif", WebkitFontSmoothing: "antialiased" }}>
            ...
            <motion.nav initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: customEase }} style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "clamp(12px, 2vh, 20px) max(5%, 24px)", paddingTop: "calc(env(safe-area-inset-top, 0px) + clamp(12px, 2vh, 20px))", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(253, 253, 253, 0.7)", backdropFilter: "blur(40px) saturate(150%)", WebkitBackdropFilter: "blur(40px) saturate(150%)", borderBottom: "1px solid rgba(0,0,0,0.04)", willChange: "transform, opacity" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <VoxaLogo size={24} /> <span style={{ fontSize: "clamp(17px, 4vw, 19px)", fontWeight: 700, letterSpacing: "-0.03em", color: themeColors.textMain }}>Voxa AI</span>
                </div>
                    ...
                    <motion.div style={{ position: "relative", y: videoContainerY, opacity: videoContainerOpacity, scale: videoContainerScale, width: "100%", maxWidth: 1100, padding: "0 max(5%, 20px)", zIndex: 10, willChange: "transform, opacity" }}>
                        <div style={{ width: "100%", background: "rgba(255, 255, 255, 0.5)", borderRadius: "clamp(24px, 5vw, 40px)", padding: "clamp(10px, 2vw, 16px)", border: "1px solid rgba(255,255,255,0.8)", boxShadow: "0 40px 80px -20px rgba(0,0,0,0.1), inset 0 1px 0 rgba(255,255,255,1), 0 0 0 1px rgba(0,0,0,0.02)", backdropFilter: "blur(40px) saturate(150%)", WebkitBackdropFilter: "blur(40px) saturate(150%)" }}>
                            <div style={{ width: "100%", aspectRatio: "16/9", background: "#05050a", borderRadius: "clamp(14px, 4vw, 24px)", boxShadow: "inset 0 2px 10px rgba(0,0,0,0.4), 0 0 0 1px rgba(0,0,0,0.05)", position: "relative", overflow: "hidden" }}>
                                <motion.video src="/Recording 2026-04-05 220447.mp4" autoPlay loop muted playsInline style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", zIndex: 0, scale: innerVideoScale, transformOrigin: "center center", willChange: "transform" }} />
                            </div>
                        </div>
                    </motion.div>
                </div>
                ...
                <div style={{ maxWidth: 1400, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 40 }}>
                    ...
                    <div style={{ display: "flex", gap: "clamp(40px, 8vw, 80px)", flexWrap: "wrap" }}>
                        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                            ...
                            <a href="#" style={{ color: themeColors.textMuted, textDecoration: "none", fontSize: 15, transition: "color 0.2s", fontWeight: 500 }}>Home</a>
                            <a href="#features" style={{ color: themeColors.textMuted, textDecoration: "none", fontSize: 15, transition: "color 0.2s", fontWeight: 500 }}>Features</a>
                            <a href="#how-it-works" style={{ color: themeColors.textMuted, textDecoration: "none", fontSize: 15, transition: "color 0.2s", fontWeight: 500 }}>How it works</a>
                            <a href="#faqs" style={{ color: themeColors.textMuted, textDecoration: "none", fontSize: 15, transition: "color 0.2s", fontWeight: 500 }}>FAQs</a>
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                            <h4 style={{ color: themeColors.textMain, fontWeight: 700, marginBottom: 8, fontSize: 15, letterSpacing: "-0.01em" }}>Legal</h4>
                            <a href="#" style={{ color: themeColors.textMuted, textDecoration: "none", fontSize: 15, transition: "color 0.2s", fontWeight: 500 }}>Privacy Policy</a>
                            <a href="#" style={{ color: themeColors.textMuted, textDecoration: "none", fontSize: 15, transition: "color 0.2s", fontWeight: 500 }}>Terms of Service</a>
                        </div>
                    </div>
                </div>
